...
class Tilemap
	...
	def load_map(filename)
		...
		@tileset = Image.load_tiles(@window, @tileset_filename, @tile[:width], @tile[:height], false)
		@map_loaded = true
	end
		...
		@layers.each { |current_layer, value|
			...
			(camera_y/@tile[:height]).upto(max_y) do |y|
				(camera_x/@tile[:width]).upto(max_x) do |x|
					...
					if tile
						@tileset[tile].draw((x * @tile[:width] - camera_x)+160, (y * @tile[:height] - camera_y)+120, @layers[current_layer][:zorder])
					end
				end
			end
		}
